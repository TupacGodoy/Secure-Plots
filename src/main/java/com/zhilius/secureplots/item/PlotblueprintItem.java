...
public class PlotblueprintItem extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (!world.isClient && user instanceof ServerPlayerEntity player) {
            ...
            if (player.isSneaking()) {
                // ── Shift+click: mostrar solo rayos, sin menú ──────────────
                ...
                // ── Click normal: abrir menú, sin rayos ───────────────────
                if (atPos != null && (atPos.getRoleOf(player.getUuid()) != PlotData.Role.VISITOR || isAdmin)) {
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltip, TooltipType type) {
        tooltip.add(Text.literal("Clic: abre el menú de gestión").formatted(Formatting.GRAY));
